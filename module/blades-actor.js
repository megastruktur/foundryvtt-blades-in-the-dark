...
export class BladesActor extends Actor {
    ...
    for (var attribute_name in this.system.attributes) {
      ...
      for (var skill_name in this.system.attributes[attribute_name].skills) {
        dice_amount[skill_name] = parseInt(
          this.system.attributes[attribute_name].skills[skill_name]["value"][0]
        );
  ...
  rollAttributePopup(attribute_name) {
    ...
    let content = `
        <h2>${game.i18n.localize("BITD.Roll")} ${game.i18n.localize(
      attribute_label
    )}</h2>
        <form>
      ...
      content += `
            <div class="form-group">
              ...
              <select id="pos" name="pos">
                <option value="controlled">${game.i18n.localize(
                  "BITD.PositionControlled"
                )}</option>
                <option value="risky" selected>${game.i18n.localize(
                  "BITD.PositionRisky"
                )}</option>
                <option value="desperate">${game.i18n.localize(
                  "BITD.PositionDesperate"
                )}</option>
              </select>
            </div>
            <div class="form-group">
              ...
              <select id="fx" name="fx">
                <option value="limited">${game.i18n.localize(
                  "BITD.EffectLimited"
                )}</option>
                <option value="standard" selected>${game.i18n.localize(
                  "BITD.EffectStandard"
                )}</option>
                <option value="great">${game.i18n.localize(
                  "BITD.EffectGreat"
                )}</option>
              </select>
            </div>`;
    ...
    new Dialog({
      title: `${game.i18n.localize("BITD.Roll")} ${game.i18n.localize(
        attribute_label
      )}`,
      content: content,
      buttons: {
